...
export async function GET(
  ...
  try {
    ...
    if (isSupabaseReady && supabaseAdmin) {
      ...
      try {
        const { data: existingArt } = await supabaseAdmin.from("user_articles").select("views_count").eq("slug", slug).maybeSingle();
        if (existingArt) {
          await supabaseAdmin.from("user_articles").update({ views_count: (existingArt.views_count || 0) + 1 }).eq("slug", slug);
        }
      } catch {
        // ignore
      }

      // 2. Ambil detail artikel + profil penulis + komentar
...
export async function POST(
  ...
  try {
    ...
    const action: "like" | "dislike" | "share" | "comment" = body.action;
